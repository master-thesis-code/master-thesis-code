...
class BertDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        description = f"title: {item['case_title']}; description: {item['ChallengeDescription']}; aim: {item['aim']}; req: {item['req']}; terminology: {item['terminology']}"
        abstrast = f"title: {item['paper_title']}; abstract: {item['abstract']};"
        label = item["label"]
...
def train(model, train_dataloader, pos_weight=1, epochs=1, start_epoch=0, bert_lr=5e-5, linear_lr=0.001, seed=404):
    ...
    for epoch in range(0, epochs):
        ...
        torch.save(model.state_dict(), f"./models/{seed}/deberta-v3-extra-data-{epoch + start_epoch}")
    end = timer()
...
def main():
    ...
    train_df = pd.read_csv("./data/df_extra/train_balanced_df.csv")
    ...
    train_df = pd.read_csv("./data/df_extra/train_less_skewed_df.csv")
    num_positives = np.sum(train_df.label)
    num_negatives = len(train_df.label) - num_positives
    pos_weight  = num_negatives / num_positives
    print(f"pos weight: {pos_weight}")
    ...
    # train one epoch on full ds
    train_df = pd.read_csv("./data/df_extra/train_df.csv")
    num_positives = np.sum(train_df.label)
    num_negatives = len(train_df.label) - num_positives
    pos_weight  = num_negatives / num_positives
    print(f"pos weight: {pos_weight}")
