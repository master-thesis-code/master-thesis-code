...
class BertDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        description = f"{item['case_title']} {item['aim']} {item['req']} {item['terminology']}"
        abstrast = f"{item['abstract']}"
        label = item["label"]
...
def train(model, train_dataloader, pos_weight=1, epochs=1, start_epoch=0, bert_lr=5e-5, linear_lr=0.001, seed=404):
    ...
    for epoch in range(0, epochs):
        ...
        torch.save(model.state_dict(), f"./models/{seed}/deberta-v3-extra-data-only-{epoch + start_epoch}")
    end = timer()
