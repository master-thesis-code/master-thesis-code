...
def train(seed, train_df, batch_size=8, epochs=1, lr=3e-5):
    ...
    model.fit(
        ...
        checkpoint_path=f"./data/checkpoints/{seed}/",
    )
    return model
...
def main():
    ...
    train_df = pd.read_csv("./data/warmup_train.csv")
    model = train(seed, train_df)
    model.save(f"./data/final_model/{seed}")
