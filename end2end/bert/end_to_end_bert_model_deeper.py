...
class EndToEndClassifier(nn.Module):
    ...
    def forward(self, description_input, abstract_input):
        ...
        l1 =  self.relu(self.linear_1(pooled_output))
        return self.linear_2(l1)
...
def train(model, train_dataloader, pos_weight=1, epochs=1, start_epoch=0, seed=404, bert_lr=5e-5, linear_lr=0.001):
    ...
    optimizer = Adam([
        ...
        {"params":model.linear_2.parameters(), "lr": linear_lr}, 
   ])
    ...
    for epoch in range(0, epochs):
        ...
        torch.save(model.state_dict(), f"./models/{seed}/bert_{epoch + start_epoch}")
    end = timer()
...
def main():
    ...
    # model.load_state_dict(torch.load(f"./models/e0-balanced"))
